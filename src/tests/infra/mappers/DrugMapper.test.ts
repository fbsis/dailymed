...
describe("DrugMapper", () => {
  ...
  describe("toDomain", () => {
    it("should convert DrugModel to Drug entity", () => {
        ...
        indications: [
          {
            code: "1.1",
            description:
              "This is a detailed test description for the condition that meets the minimum length requirement for the description field in the database",
          },
        ],
        dosage: {
          value: "300 mg",
      ...
      expect(result.getIdentificationCode().getValue()).toBe(
        drugModel.identificationCode
      );

      const indications = result.getIndications();
      expect(indications).toHaveLength(1);
      expect(indications[0].getCode().getValue()).toBe(
        drugModel.indications[0].code
      );
      expect(indications[0].getDescription().getValue()).toBe(
        drugModel.indications[0].description
      );

      const dosage = result.getDosage();
      expect(
        dosage.getImportantAdministrationInstructions()[0].getValue()
      ).toBe(drugModel.dosage.value);
      expect(
        dosage
          .getAgeGroups()
          .getAllAgeBasedDosages()[0]
          .getAgeRange()
          .toString()
      ).toBe("0 to 12 years");
    });
    ...
    it("should handle null values in optional fields", () => {
      ...
      expect(
        result.getDosage().getImportantAdministrationInstructions()
      ).toHaveLength(1);
    });
  });
    ...
    it("should convert Drug entity to DrugModel", () => {
      ...
      const indications = [
        new Indication(
          ...
          new Description(
            "This is a detailed test description that meets the minimum length requirement"
          )
        ),
      ];
